...
registerCommand(registerInformation, function* (session, builder, args) {
    ...
    const radius = args.get('radius') < 0 ? Infinity : args.get('radius');
    ...
    for (const entity of dimension.getEntities(entityQuery)) {
        ...
        if (allOff)
            matches = true;
        else if (flags.g && entity.id.match(/golem/))
            matches = true;
        else if (flags.t && entity.nameTag)
            matches = true;
        else if (flags.r && entity.id == 'minecraft:armor_stand')
            matches = true;
        else if (flags.n && entity.id.match(/(villager)|(wandering_trader)|(npc)/))
            matches = true;
        else if (flags.p && entity.hasComponent('minecraft:is_tamed'))
            matches = true;
        else if (flags.a && animals.includes(entity.id))
            matches = true;
        else if (flags.b && ambientMobs.includes(entity.id))
            matches = true;
        else if (flags.w && waterMobs.includes(entity.id))
            matches = true;
        if (matches) {
            try {
                entity.kill();
