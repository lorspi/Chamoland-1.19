...
export function regionTransformedBounds(start, end, origin, rotate, flip) {
    const corners = [
 ...
 * @param end
 */
export function* regionIterateBlocks(start, end) {
    ...
    for (let z = 0; z < size.z; z += maxSize)
        for (let y = 0; y < size.y; y += maxSize)
            for (let x = 0; x < size.x; x += maxSize) {
                ...
                const subEnd = Vector.min(new Vector(x, y, z).add(maxSize), size).add(min).sub(Vector.ONE).toBlock();
